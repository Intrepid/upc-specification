...
\np \xadded[id=DB]{91}{
     ...
     specified in Section~\ref{upc-io}
     are declared by the header {\tt <upc\_io.h>}.
    }

\np \xadded[id=DB]{10}{
    Every inclusion of {\tt <upc\_io.h>} has the effect of including {\tt <upc\_types.h>}.
    }
...
UPC-IO data movement library functions is explicitly controlled by using the 
{\tt upc\_flag\_t flags} argument. See \upcflagsection for details.}
...
\np All UPC-IO read/write functions take an argument {\tt flags}
    ...
    defined in \upcflagsection. These semantics apply only to
