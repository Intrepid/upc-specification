...
\usepackage[
  ...
  colorlinks=true]{hyperref}  % should come last after all other usepackages
...
\np This document focuses only on the UPC specifications
...
\np For the purpose of these specifications the following
    definitions apply.
...
\subsection*{3.1}
\index{thread}
\npf {\bf thread}\\ an instance of execution initiated by the execution
  environment at program startup.

\subsection*{3.2}
\index{object}
\npf {\bf object}\\ region of data storage in the execution environment
   which can represent values.

\subsubsection*{3.2.1}
\index{shared object}
\npf {\bf shared object}\\ an object allocated using a shared-qualified
...
\subsubsection*{3.2.2}
\index{private object}
\npf {\bf private object}\\ any object which is not a shared object.

\np NOTE \hspace{5pt} Each thread declares and creates its own 
    private objects which no other thread can access.\footnote{The file scope declaration
    {\tt int y;} creates a separate object for each thread to access.}

\subsubsection*{3.2.3}
\index{shared array}
\npf{\bf shared array}\\ an array with elements that have shared qualified
  type.
  
\subsection*{3.3}
\index{affinity}
\npf {\bf affinity}\\ logical association between shared
   objects and threads.  Each element of data storage that contains shared objects
   has affinity to exactly one thread.
 
\subsection*{3.4}
\index{pointer-to-shared}
\npf {\bf pointer-to-shared}\\  a pointer whose referenced type is shared-qualified.

\subsection*{3.5}
\index{pointer-to-local}
\npf {\bf pointer-to-local}\\  a pointer whose referenced type is not shared-qualified.

\subsection*{3.6}
\label{def-access}
\index{access}
\npf {\bf access}\\ $<$execution-time action$>$ to read or
    modify the value of an object by a thread.

\subsubsection*{3.6.1}
\index{shared access}
\npf {\bf shared access}\\ an access using an expression whose type
    is shared-qualified.

\paragraph*{3.6.1.1}\ \\
\index{strict shared read}
\npf {\bf strict shared read}\\ a shared read access which is determined to be
    strict according to section \ref{type_qualifiers} of this specification.

\paragraph*{3.6.1.2}\ \\
\index{strict shared write}
\npf {\bf strict shared write}\\ a shared modify access which is determined to be
    strict according to section \ref{type_qualifiers} of this specification.

\paragraph*{3.6.1.3}\ \\
\index{relaxed shared read}
\npf {\bf relaxed shared read}\\ a shared read access which is determined to be
    relaxed according to section \ref{type_qualifiers} of this specification.

\paragraph*{3.6.1.4}\ \\
\index{relaxed shared write}
\npf {\bf relaxed shared write}\\ a shared modify access which is determined to be
    relaxed according to section \ref{type_qualifiers} of this specification.

\subsubsection*{3.6.2}
\index{local access}
\npf {\bf local access}\\ an access using an expression whose type
    is not shared-qualified.

\subsection*{3.7}
\index{collective}
\index{synchronization}
\npf {\bf collective}\\ a constraint placed on some language 
    operations which requires evaluation of such operations to be
    matched\footnote{A collective operation need not provide any actual
    synchronization between threads, unless otherwise noted.  The collective
    requirement simply states a relative ordering property of calls to collective
    operations that must be maintained in the parallel execution trace for all
    executions of any legal program.  Some implementations may include
    unspecified synchronization between threads within collective operations,
    but programs must not rely upon such unspecified synchronization for
    correctness.} across all threads.
    The behavior of collective operations is undefined
    unless all threads execute the same sequence of collective operations.

\subsection*{3.8}
\index{single-valued}
\npf {\bf single-valued}\\ an operand to a collective
    operation, which has the same value on every thread.  The behavior
    of the operation is otherwise undefined.

\subsection*{3.9}
\index{phase}
\npf {\bf phase}\\ an unsigned integer value associated with a pointer-to-shared which
   indicates the element-offset within an affinity block; used in pointer-to-shared 
   arithmetic to determine affinity boundaries.
...
\index{main}
\np In the execution environment of a UPC program, derived
...
\index{exit}
\np A program is terminated by the termination of all the
...
\np Thread termination follows the C Standard definition of
    ...
    terminated by reaching the \verb }  that terminates the main
...
\index{program order}
\index{data races}
\np Thread execution follows the C Standard definition of
...
\label{threads}
\index{THREADS}
\np {\tt THREADS} is an expression with a value of type int; it specifies the number of
...
\index{MYTHREAD}
\np {\tt MYTHREAD} is an expression with a value of type int; it specifies the
...
\index{UPC\_MAX\_BLOCK\_SIZE}      
\index{block size}
\np {\tt UPC\_MAX\_BLOCK\_SIZE} is a predefined integer
...
\index{sizeof}
\np The {\tt sizeof} operator will result in an
...
\index{upc\_localsizeof}
\np The {\tt upc\_localsizeof} operator shall apply only to
...
\index{upc\_blocksizeof}
\np The {\tt upc\_blocksizeof} operator shall apply only to shared-qualified
...
\index{upc\_elemsizeof}
\np The {\tt upc\_elemsizeof} operator shall apply only to shared-qualified
     expressions or shared-qualified types.  All constraints on the {\tt sizeof} operator
     [ISO/IEC00 Sec. 6.5.3.4] also apply to this operator.
...
\np No binary operators shall be applied to one pointer-to-shared
...
\np When the unary {\tt \&} is applied to a shared structure
    element of type {\tt T}, the result has type {\tt shared [] T *}.

\subsection{Declarations}

\np UPC extends the declaration ability of C to allow shared
...
\np The following is the declarator definition as per
...
\np {\em synchronization-statement:}
...
\np The UPC memory allocation functions return, if successful,
...
\np {\tt threadid} shall be a value in 0..({\tt THREADS}-1).
...
\np {\tt upc\_flag\_t} is an integral type defined in {\tt<upc.h>} which is used to
...
that has affinity to thread $i$ pointed to by {\tt src}
...
\index{upc\_off\_t}
\np The following types are defined in {\tt <upc\_io.h>}
...
\index{common file pointer}
\index{individual file pointer}
\np When a file is opened with an individual file pointer, each thread will get
...
\np List I/O functions take a list of addresses/offsets and corresponding
...
\label{mem-semantics}
\index{memory consistency}
\np The memory consistency model in a language defines the order in 
...
\np UPC provides several synchronization operations 
...
\np The memory model definition is rather subtle in some points, but
...
\np The subsequent examples demonstrate the semantics of the memory
...
\index{Precedes}
\index{program order}
\np This section outlines a formal definition for the $Precedes(m_1,m_2)$
