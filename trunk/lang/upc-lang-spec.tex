...
\sterm{thread}{3.1} an instance of execution initiated by the execution
  environment at program startup.

\sterm{object}{3.2} region of data storage in the execution environment
  which can represent values.

\ssterm{shared object}{3.2.1} an object allocated using a shared-qualified
...
\ssterm{private object}{3.2.2} any object which is not a shared object.
...
\ssterm{shared array}{3.2.3} an array with elements that have shared qualified type.
  
\sterm{affinity}{3.3} logical association between shared
  objects and threads.  Each element of data storage that contains
  shared objects has affinity to exactly one thread.
 
\sterm{pointer-to-shared}{3.4} a pointer whose referenced type is shared-qualified.

\sterm{pointer-to-local}{3.5} a pointer whose referenced type is not
  shared-qualified.

\label{def-access}
\sterm{access}{3.6} $<$execution-time action$>$ to read or
  modify the value of an object by a thread.

\ssterm{shared access}{3.6.1} an access using an expression whose type
  is shared-qualified.

\pterm{strict shared read}{3.6.1.1} a shared read access which is
  determined to be strict according to section \ref{type_qualifiers} of
  this specification.

\pterm{strict shared write}{3.6.1.2} a shared modify access which is
  determined to be strict according to section \ref{type_qualifiers} of
  this specification.

\pterm{relaxed shared read}{3.6.1.3} a shared read access which is
  determined to be relaxed according to section \ref{type_qualifiers} of
  this specification.

\pterm{relaxed shared write}{3.6.1.4} a shared modify access which is
  determined to be relaxed according to section \ref{type_qualifiers} of
  this specification.

\ssterm{local access}{3.6.2} an access using an expression whose type
  is not shared-qualified.

\index{synchronization}
\sterm{collective}{3.7} constraint placed on some language 
...
\ssterm{single-valued}{3.7.1} an operand to a collective operation, which has
  the same value on every thread.  The behavior of the operation
  is otherwise undefined.

\ssterm{phase}{3.7.2} an unsigned integer value associated with a
