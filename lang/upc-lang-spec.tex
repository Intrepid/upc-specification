...
\newcommand{\sterm}[1]{\subsection{#1}\index{#1}}
\newcommand{\ssterm}[1]{\subsubsection{#1}\index{#1}}
\newcommand{\pterm}[1]{\paragraph{#1}\index{#1}}
...
\npf an instance of execution initiated by the execution
     environment at program startup.

\sterm{object}

\npf region of data storage in the execution environment
     which can represent values.

\ssterm{shared object}

\npf an object allocated using a shared-qualified
...
\npf any object which is not a shared object.
...
\npf an array with elements that have shared qualified type.
  
\sterm{affinity}

\npf logical association between shared
...
\npf a pointer whose referenced type is shared-qualified.

\sterm{pointer-to-local}

\npf a pointer whose referenced type is not
     shared-qualified.

\label{def-access}
\sterm{access}

\npf $<$execution-time action$>$ to read or
     modify the value of an object by a thread.

\ssterm{shared access}

\npf an access using an expression whose type
     is shared-qualified.

\pterm{strict shared read}

\npf a shared read access which is determined to be
     strict according to section \ref{type_qualifiers} of this specification.

\pterm{strict shared write}

\npf a shared modify access which is determined to be
     strict according to section \ref{type_qualifiers} of this specification.
...
\npf constraint placed on some language 
...
\ssterm{single-valued}

\npf an operand to a collective operation, which has
     the same value on every thread.  The behavior of the operation
     is otherwise undefined.

\ssterm{phase}

\npf an unsigned integer value associated with a
