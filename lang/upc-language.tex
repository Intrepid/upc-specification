...
\npf The {\tt upc\_localsizeof} operator shall apply only to
     \xreplaced[id=SV]{3}{a shared type or an expression with shared type}
...
\np The {\tt upc\_localsizeof} operator returns the size, in
     ...
     object or a \xreplaced[id=SV]{3}{shared}{shared-qualified} type. 
...
\np The {\tt upc\_blocksizeof} operator returns the block
     size of \xreplaced[id=SV]{3}{the ultimate element type of the operand}
...
\np The {\tt upc\_elemsizeof} operator returns the size, in bytes,
     of the \xreplaced[id=SV]{3}{ultimate element type of its operand.}
     {highest-level (leftmost) type that is not an array.}  For
     \xreplaced[id=SV]{3}{a non-array operand}{non-array objects}, 
...
\begin{verbatim}
    ...
    p1 = p + i;  
\end{verbatim}
\xchangenote[id=SV]{3}{Changed referenced type of pointer.}
...
\begin{verbatim}
    ...
    upc_threadof(p1) == (upc_threadof(p)
                        ...
                        mod THREADS
\end{verbatim}
\xchangenote[id=SV]{3}{Changed formulas to support multi-dimensional shared arrays.}    
...
\end{verbatim}    
\xchangenote[id=DB]{106}{Declaration of S1 and S1 changed to indefinite blocksize to accommodate new constraint. Comments clarified.}
...
\truefootnote{This implies there is no padding inserted between blocks of shared array elements with affinity to a thread.}
}
\end{itemize}
...
\np When two pointers-to-shared are compared using a relational operator, 
    ...
    If the result of the subtraction is undefined, so is the result of the relational operator.
}

{\bf Forward references:}  {\tt upc\_threadof} (\ref{upc_threadof}),
          {\tt upc\_phaseof} (\ref{upc_phaseof}). 
...
\npf A shared type qualifier shall not appear in a type cast
    ...
    being cast is not \xreplaced[id=SV]{3}{a shared type}{shared-qualified}.
    \footnote{i.e., pointers-to-local
    cannot be cast to pointers-to-shared.}  An exception is made when 
    \xreplaced[id=DB]{65}{a null pointer constant}{the constant expression 0}
...
\np If a non-null pointer-to-shared is cast\footnote{As such pointers
     ...
     of \xadded[id=SV]{3}{any byte comprising} the
...
\np \xreplaced[id=SV]{3}{Bytes}{Shared objects} with
    affinity to a given thread \xadded[id=SV]{3}{containing shared objects}
...
\np \xadded[id=DB]{71}{
    A layout qualifier of [*] shall not appear in the declaration specifiers of
    a declaration whose storage-class specifier is typedef.
    }
...
\np The layout qualifier dictates the blocking factor for
    ...
    number of consecutive \xreplaced[id=SV]{3}{objects with ultimate
    element type of the array}{elements} (when evaluating pointer-to-shared
...
\np \xadded[id=DB]{110}{
    ...
    with the top-level (rightmost) shared qualifier removed.%
      \truefootnote{For example, in the file-scope declaration {\tt shared [10] int A[10*THREADS];}
       the effective type of the object {\tt A[0]} is {\tt int}.
       This implies the following lvalue expressions are all permitted for accessing the object:
       % DOB: would like to use verbatim environment here, but Latex does not allow it in a footnote
       \begin{quote}
       {\tt int x1 = A[0];}\\
...
\np No object with automatic storage duration shall have \xreplaced[id=SV]{3}
...
\np 
  ...
  \xreplaced[id=DB]{95}{%
    ...
    Every declaration of a shared array with definite blocksize shall include the
    THREADS keyword exactly once, in one dimension of the array 
    (including through typedefs).  
    Every array type that is a shared type with definite blocksize
    shall include the THREADS keyword at most once, in one dimension
    (including through typedefs).  
  }{%
  ...
  \xreplaced[id=DB]{94}{%
    In both cases, the THREADS keyword shall only occur either alone or when
...
\np \xadded[id=DB]{15}{
    The THREADS keyword
    shall not appear in any array type that is a shared array with 
    indefinite blocksize under the {\em dynamic THREADS} environment.
    }

\np \xadded[id=DB]{78}{
    If an init-declarator that declares a shared array
    includes an initializer, the behavior is implementation-defined.
    }
...
\np \xreplaced[id=SV]{3}
    ...
    {Elements of shared arrays are distributed in a round
    robin fashion, by chunks of block-size elements, such that the
    i-th element} has affinity with thread \xreplaced[id=SV]{3}
    {($\lfloor i/block\_size \rfloor$ {\tt mod THREADS})}
    {(floor (i/block\_size) {\tt mod THREADS})}.

\np In an array declaration, the type qualifier applies to
    the \xreplaced[id=SV]{3}{ultimate element type of the
    array}{elements}.
...
\end{verbatim}
   ...
   {\tt shared [3]} applies to the \xreplaced[id=SV]{3}{ultimate
   element type}{underlying type} of T, which is int,
...
\np When {\em affinity} is an integer expression, the loop body of
    ...
    \xadded[id=DB]{59}{If the value of {\em affinity} is negative, behavior is undefined.}
...
\end{verbatim}
   ...
   \xadded[id=DB]{29}{followed by a final call to foo2() on each thread before the loop terminates.}
...
\xreplaced[id=DB]{105}{201309L}{200505L}.
