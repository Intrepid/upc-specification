...
int main(int argc, char **argv) {
  ...
  for (int iter=0; iter < iters; iter++) {
    ...
    for (int region = 0; region < sizeof(my_regions)/sizeof(my_regions[0]); region++) {
      shared [] T *regionp = my_regions[region];
      assert(upc_threadof(regionp) == MYTHREAD);
      for (int i=0; i < BS; i++) {
        shared [] T *sp = regionp + i;
        assert(upc_threadof(sp) == MYTHREAD);
        T *lp = (T *)sp;
        assert(lp != NULL);
        T *cp = upc_cast(sp);
  ...
  for (int iter=0; iter < iters; iter++) {
    ...
    for (int th = (MYTHREAD+1)%THREADS; th != MYTHREAD; th = (th + 1)%THREADS) {
      ...
      for (int region = 0; region < sizeof(th_regions)/sizeof(th_regions[0]); region++) {
        shared [] T *regionp = th_regions[region];
        assert(upc_threadof(regionp) == th);
        T *base = upc_cast(regionp);
        int castable = (base != NULL);
  rescan:
        for (int i=0; i < BS; i++) {
          shared [] T *sp = regionp + i;
          assert(upc_threadof(sp) == th);
          T *cp = upc_cast(sp);
          total++;
          if (castable && cp == NULL) {
